import { Button, cn } from "@unimart/ui";
import {
  CheckCircle2,
  Flashlight,
  FlashlightOff,
...
function HandshakeQR({
  ...
  return (
    <div
      className={cn(
        "relative flex flex-col items-center justify-between rounded-3xl border border-border bg-card p-8 text-center shadow-sm overflow-hidden min-h-[480px] w-full max-w-sm mx-auto space-y-4",
        className
      )}
      {...props}
    >
      <div className="w-full space-y-1">
        <h3 className="text-lg font-heading font-bold text-foreground">Scan to Confirm</h3>
        <p className="text-sm text-muted-foreground">Show this code to the buyer</p>
      </div>
      ...
      <div className="relative group p-8 bg-card rounded-3xl shadow-inner border-2 border-primary/10">
        <div
          className={cn(
            ...
            isExpired || timeLeft === 0 ? "opacity-100" : "opacity-0 pointer-events-none"
          )}
        >
        ...
        <div className="absolute inset-0 border-[3px] border-dashed border-primary/30 rounded-3xl pointer-events-none" />
      </div>
      ...
      <div className="w-full space-y-4">
        <div className="bg-primary/5 rounded-2xl p-4 border border-primary/10">
          ...
          <div className="flex justify-between items-center">
            <span className="font-bold text-foreground truncate max-w-[180px]">{itemTitle}</span>
        ...
        <div className="space-y-2">
          <div className="flex justify-between text-xs font-medium text-muted-foreground">
            ...
            <span className={cn("tabular-nums", timeLeft < 10 && "text-destructive")}>
...
function HandshakeScanner({ className, isScanning = true, ...props }: HandshakeScannerProps) {
  const [torchOn, setTorchOn] = React.useState(false);

  return (
    <div
      className={cn(
        "relative flex flex-col rounded-3xl border border-border bg-card overflow-hidden min-h-[480px] w-full max-w-sm mx-auto",
        className
      )}
      {...props}
    >
...
function HandshakeSuccess({ className, transactionId, onRate, ...props }: HandshakeSuccessProps) {
  return (
    <div
      className={cn(
        "relative flex flex-col items-center justify-center rounded-3xl border border-border bg-card/50 backdrop-blur-xl overflow-hidden min-h-[480px] w-full max-w-sm mx-auto p-8 text-center",
        className
      )}
      {...props}
    >
      ...
      <div className="relative z-10 flex flex-col items-center space-y-6 animate-in zoom-in-95 duration-500">
        <div className="relative">
          ...
          <div className="relative h-24 w-24 rounded-full bg-primary flex items-center justify-center shadow-lg shadow-primary/30">
            <CheckCircle2 className="h-12 w-12 text-primary-foreground" strokeWidth={3} />
          </div>
        </div>

        <div className="space-y-2">
          <h3 className="text-2xl font-heading font-bold text-foreground">Trade Confirmed!</h3>
